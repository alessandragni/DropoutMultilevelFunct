simulate_once <- function(seed, I = 20, J = 4, grid = 1000, 
  ...
  simulated_data <- simulated_data %>%
    group_by(id, centre) %>%
    dplyr::mutate(enum = row_number() - 1,
           timecum = time,
           event = ifelse(timecum == max(timecum), 0, 1),
           time = ifelse(enum == 0, timecum, timecum - lag(timecum, n=1))
    ) %>%
    ungroup() %>%
    dplyr::mutate(id = dense_rank(id) + max(id) * (centre - 1) )
  
  simulated_data <- simulated_data %>%
    group_by(id, centre) %>%
    dplyr::mutate(logp1_enum = log(enum+1),
  ...
  simulated_data <- simulated_data %>%
    dplyr::mutate(timecum = grid * timecum,
